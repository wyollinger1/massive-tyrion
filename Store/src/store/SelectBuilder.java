...
public class SelectBuilder {
  ...
  private final static String[]        OPS_SET_VALS = { "LIKE", "IN",
      "BETWEEN", "=", "<>", ">=", "<="             };
   ...
   * 
   * @param colName
   *          String name of the column to apply the condition - no sanitization
   *          on colName
   * @param operator
   *          String name of the operator, not IN or BETWEEN which are special
   * @param conditionArr
   *          String array condition applied, NOT NULL
   * @param isAnd
   *          true if AND'ing in the condition otherwise false - meaningless for
   *          the first condition
   * @return Format string for a PreparedStatement
   * @throws SQLException
   *           if size of conditionArr does not match what is expected by the
   *           operator.
   * @throws Exception
   *           if the parameter, operator, is not a recognized operator
   */
  private String makeFormatString(String colName, String operator,
      Object[] conditionArr, boolean isAnd) throws SQLException, Exception {
  ...
  public void addStringCondition(String colName, String operator,
      String[] conditionArr, boolean isAnd) throws SQLException, Exception {
    String condFormatStr = makeFormatString(colName, operator, conditionArr,
        isAnd);
  ...
  public void addIntCondition(String colName, String operator,
      Integer[] conditionArr, boolean isAnd) throws SQLException, Exception {
    String condFormatStr = makeFormatString(colName, operator, conditionArr,
        isAnd);
  ...
  public void addDoubleCondition(String colName, String operator,
      Double[] conditionArr, boolean isAnd) throws SQLException, Exception {
    String condFormatStr = makeFormatString(colName, operator, conditionArr,
        isAnd);
   ...
   * @throws SQLException
   *           if something goes wrong with the internal PreparedStatement or
  ...
  public ResultSet executeSelect(Connection con) throws SQLException {
    ...
    assert numParams == whereStrArr.size() : String.format(
        "numParams: %d is not the same as whereStrArr.size: %d", numParams,
        whereStrArr.size());
