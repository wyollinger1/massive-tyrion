    ...
    if (user != null && user instanceof Manager) {
      ...
      if (Util.validateParams(pm, "id", "num")) {
        ...
        store.Media newMedia = Util.correctType(pm.get("creator")[0],
            pm.get("name")[0], Integer.parseInt(pm.get("duration")[0]),
            pm.get("genre")[0], Double.parseDouble(pm.get("price")[0]),
            Integer.parseInt(pm.get("numRatings")[0]),
            Double.parseDouble(pm.get("avgRating")[0]), 0, pm.get("type")[0]);
        if (newMedia != null) {
          addedMedia = man.addMedia(newMedia,
              Integer.parseInt(pm.get("num")[0]));
  ...
  protected void doDelete(HttpServletRequest request,
    ...
    if (user != null & user instanceof Manager) {
      if (Util.validateParams(pm, "id", "num")) {
        // Utility variable to hold user as a manager
        Manager man = (Manager) user;
        store.Media toDelete = DBIO.getMedia(Integer.parseInt(pm.get("id")[0]));
        if (toDelete != null) {
